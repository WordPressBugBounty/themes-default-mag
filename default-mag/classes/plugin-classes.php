<?php
if ( !class_exists('DefaultMag_Getting_started') ):

    class DefaultMag_Getting_started
    {   

        function __construct()
        {	

	        add_action( 'wp_ajax_default_mag_install_plugins', array( $this, 'default_mag_install_plugins' ) );
        ...
        public static function default_mag_plugin_status($plugin_class, $plugin_folder, $plugin_file){

            if( class_exists( $plugin_class ) ){

                return array('status' => 'active','string' => esc_html__('Deactivate','default-mag') );
                ...
                if( file_exists( $path ) ) {
                    ...
                    return array('status' => 'deactivate','string' => esc_html__('Activate','default-mag') );
                }else{

                    return array('status' => 'not-install','string' => esc_html__('Install & Active','default-mag') );
        ...
        public static function default_mag_recommended_plugins(){
            ...
            return $plugin_lists = array(
                'social-share-with-floating-bar' => array(
                    ...
                    'setting_page' => esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=social-share-with-floating-bar' ),
                ),
        ...
        public function default_mag_install_plugins(){
            
            $nonce = isset( $_POST["_wpnonce"] ) ? sanitize_text_field( wp_unslash( $_POST["_wpnonce"] ) ) : '';

            if ( ! current_user_can('install_plugins') ) {
                wp_die( esc_html__( 'Sorry, you are not allowed to install plugins on this site.', 'default-mag' ) );
            }

            // Check our nonce, if they don't match then bounce!
            if (! wp_verify_nonce( $nonce, 'default_mag_ajax_nonce' )) {

                wp_die( esc_html__( 'Error - unable to verify nonce, please try again.', 'default-mag') );
            ...
            if( isset( $_POST['single'] ) ){
                ...
                $plugin_lists = $this->default_mag_recommended_plugins();
            ...
            foreach( $plugin_lists as $key => $plugin ){
                ...
                if( isset( $_POST['single'] ) ){
                    ...
                    $pluginstatus   = $this->default_mag_plugin_status( $plugin['class'],$key,$plugin['PluginFile'] );
                ...
                if( isset( $_POST['single'] ) && $PluginStatus == 'active' ) {
                    ...
                    esc_html_e('Deactivated' , 'default-mag');
                    die();
                    ...
                    if( isset( $_POST['single'] ) ) {
                        ...
                        esc_html_e('Activated' , 'default-mag');
                        die();
                    ...
                    if( file_exists($plugin_dir) ) {
                        ...
                        if( isset( $_POST['single'] ) ) {
                            esc_html_e('Installed & Activated' , 'default-mag');
                            die();
                        }
            ...
            if( !isset( $_POST['single'] ) ) {
                echo esc_url( get_home_url(null, '/').'wp-admin/themes.php?page=default-mag-about' );
            }
    ...
    new DefaultMag_Getting_started();
