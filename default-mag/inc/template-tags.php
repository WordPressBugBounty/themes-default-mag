	...
	function default_mag_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="twp-categories twp-links"><span class="twp-icon"><i class="fa fa-calendar"></i></span>' . esc_html__( 'Posted in %1$s', 'default-mag' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
        ...
        if ('post' == get_post_type($post->ID)) {
            ...
            <span class="twp-single-post-author">
                <a href="<?php echo esc_url(get_author_posts_url($author_id)) ?>">
                    <span class="twp-author-image"><?php echo get_avatar(get_the_author_meta('ID'), '96', '', '', array('decoding' => 'async')); ?></span>
        ...
        if ('post' === get_post_type()) {
            ...
            if ($post_categories) {
                ...
                foreach ($post_categories as $post_category) {
                    $output .= '<a href="' . esc_url(get_category_link($post_category)) . '" alt="' . esc_attr(sprintf(__('View all posts in %s', 'default-mag'), $post_category->name)) . '"> 
